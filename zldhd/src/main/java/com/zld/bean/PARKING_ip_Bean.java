...
public class PARKING_ip_Bean {
    ...
    public static String typeIP(int type){
        ...
        Log.v("39999","自动拍照---:"+PARKING_ip.toString());

        for (int i =0;PARKING_ip!=null && i<PARKING_ip.size();i++){
            Log.v("39999","自动拍照i---p:"+PARKING_ip.get(i).getIp());
            switch (type){
    ...
    public static int isType(String ip){
        ...
        Log.v("39999",ip+"自动拍照---:"+PARKING_ip.toString());

        for (int i =0;PARKING_ip!=null && i<PARKING_ip.size();i++){

            Log.v("自动拍照---:",PARKING_ip.get(i).getPasstype().equals("0") +"--"+ ip.equals(PARKING_ip.get(i).getIp()));
