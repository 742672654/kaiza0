...
public class SocketUtil {
	...
	public void sendLedData(MyLedInfo myLedInfo, final String uid,
		...
		//根据ip推送显示的内容
		if(myLedInfo!=null && myLedInfo.getLedip()!=null && !myLedInfo.getLedip().equals(""))fas(myLedInfo.getLedip(),cashOrderData);
		Log.e(TAG,"cashOrderData="+cashOrderData+"     ,content="+content);
//		Log.e(TAG, "显示LEDInfo："+myLedInfo.toString()+"/n"+ "uid="+ uid +"; content"+ content+"; cashOrderData="+ cashOrderData+";  playPeaker"+ playPeaker+"播报内容"+cashOrderData+"playPeaker");
		...
		try {
			...
			if(myLedInfo != null){
				...
				FileUtil.writeSDFile("SocketUtil Led:", "myLedInfo ip:"+ledIp);
				Log.e("SocketUtil Led:", "myLedInfo ip:"+ledIp);
				...
				FileUtil.writeSDFile("SocketUtil Led:", "myLedInfo == null");
				String ledIp = uid;
    ...
    public  void fas(String ip,String cashOrderData){


		new LedThread(ip,cashOrderData).start();
