 ...
 * 设置工作站
 */
public class ChooseWorkstation2Activity extends FragmentActivity {

	public static final String TAG = "ChooseWorkstationActivity";

	private TextView   tv_pole_worktype, tv_softkeyboard, tv_fuzy_search,tv_month_card;
...
//		initIntentStaname();
	...
	private void initView() {
		...
		if(getWifiName() != null){
			et_wifi.setText(getWifiName().replace("\"", "").length()>=10?getWifiName().replace("\"", "").substring(0,10):getWifiName().replace("\"", ""));
		}
		...
		if (isPoleAutoWorking()){
			...
			tv_pole_worktype.setTextColor(getResources().getColor(R.color.black));
		}else {
			cb_pole_worktype.setChecked(false);
		}
		if (isShowSoftKeyBoard()){
			cb_softkeyboard.setChecked(true);
			tv_softkeyboard.setTextColor(getResources().getColor(R.color.black));
		}else {
		...
		if (showMonthCard){
			...
			tv_month_card.setTextColor(getResources().getColor(R.color.black));
		}else {
...
//		});
		...
		btn_into_background.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				ChooseWorkstation2Activity.this.startActivity(
						new Intent(ChooseWorkstation2Activity.this,BackgroundActivity.class));
			}
		});
		//确认
	...
	private void initSqliteManager() {
		if (sqliteManager == null) {
			sqliteManager = ((application) getApplication())
					.getSqliteManager(ChooseWorkstation2Activity.this);
		}
	}
